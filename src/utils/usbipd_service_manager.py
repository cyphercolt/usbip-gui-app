...
class USBIPDServiceManager:
    ...
    @staticmethod
    def get_service_startup_type(client: paramiko.SSHClient) -> Tuple[Optional[str], str]:
        """
    ...
    def set_service_startup_auto(client: paramiko.SSHClient) -> Tuple[bool, str]:
        ...
        try:
            ...
            stdin, stdout, stderr = client.exec_command('sc config usbipd start= auto', timeout=10)
            output = stdout.read().decode() + stderr.read().decode()
    ...
    @staticmethod
    def install_usbipd_check(client: paramiko.SSHClient) -> Tuple[bool, str, Optional[str]]:
        """
