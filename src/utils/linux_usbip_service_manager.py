...
class LinuxUSBIPServiceManager:
    ...
    def check_service_status(ssh_client, password=None):
        ...
        try:
            ...
            for i, line in enumerate(log_lines):
                if 'listening on' in line.lower() and ('3240' in line or '0.0.0.0' in line or ':::' in line):
            ...
            listening_on_port = last_listening_line > last_stopped_line and last_listening_line >= 0
            ...
            if active_line:
                ...
                if listening_on_port:
                    ...
                    daemon_status_msg = "ℹ️ usbipd daemon: STOPPED (start if you need to share devices)"
                    daemon_running = False
                elif 'active (running)' in active_lower:
                    daemon_status_msg = "� usbipd daemon: RUNNING (can share devices)"
                    daemon_running = True
                elif 'activating' in active_lower:
                    daemon_status_msg = "🟡 usbipd daemon: STARTING (can share devices soon)"
                    daemon_running = False  # Not fully operational yet
                elif 'deactivating' in active_lower:
                    daemon_status_msg = "� usbipd daemon: STOPPING (service shutting down)"
                    ...
                    daemon_status_msg = f"⚠️ usbipd daemon: UNKNOWN STATUS ({active_line})"
                    daemon_running = False
            else:
            ...
            for service_name in ["usbipd", "usbip", "usbip-daemon"]:
                stdin, stdout, stderr = ssh_client.exec_command(f"systemctl is-enabled {service_name} 2>/dev/null || echo 'disabled'")
            ...
            if enabled_status == "enabled":
                ...
                status_parts.append("ℹ️ usbipd auto-start: DISABLED (enable if you need to share devices)")
            
            # Check kernel modules (REQUIRED for attaching devices)
            stdin, stdout, stderr = ssh_client.exec_command("lsmod | grep -E 'usbip_host|usbip_core'")
            modules_output = stdout.read().decode().strip()
            
            if "usbip_host" in modules_output and "usbip_core" in modules_output:
                status_parts.append("🟢 USB/IP kernel modules: LOADED (can attach devices)")
                modules_loaded = True
            else:
                status_parts.append("🔴 USB/IP kernel modules: NOT LOADED (cannot attach devices)")
    ...
    def start_service(ssh_client, password):
        ...
        try:
            ...
            modprobe_cmd = SecureCommandBuilder.build_modprobe_command("usbip_host usbip_core", password, remote_execution=True)
            if modprobe_cmd:
            ...
            for service_name in ["usbipd", "usbip", "usbip-daemon"]:
                stdin, stdout, stderr = ssh_client.exec_command(f"systemctl list-unit-files | grep {service_name} | head -1")
                if stdout.read().decode().strip():
                    service_to_start = service_name
                    break
            
            # Start usbipd service using secure command builder
            start_cmd = SecureCommandBuilder.build_systemctl_command("start", service_to_start, password, remote_execution=True)
            if not start_cmd:
            ...
            if stderr_output and "failed" in stderr_output.lower():
                return False, f"Failed to start {service_to_start} service: {stderr_output}"
            ...
            is_operational, status_message, daemon_running = LinuxUSBIPServiceManager.check_service_status(ssh_client, password)
    ...
    def stop_service(ssh_client, password):
        ...
        try:
            ...
            stop_cmd = SecureCommandBuilder.build_systemctl_command("stop", "usbipd", password, remote_execution=True)
            if not stop_cmd:
                return False, "Failed to build secure stop command - check SecureCommandBuilder service whitelist"
            ...
            if stderr_output and ("failed" in stderr_output.lower() or "error" in stderr_output.lower()):
    ...
    def enable_auto_start(ssh_client, password):
        ...
        """
        try:
            enable_cmd = SecureCommandBuilder.build_systemctl_command("enable", "usbipd", password, remote_execution=True)
            if not enable_cmd:
    ...
    def disable_auto_start(ssh_client, password):
        ...
        """
        try:
            disable_cmd = SecureCommandBuilder.build_systemctl_command("disable", "usbipd", password, remote_execution=True)
            if not disable_cmd:
    ...
    def check_installation(ssh_client):
        ...
        try:
            ...
            stdin, stdout, stderr = ssh_client.exec_command("usbip version 2>/dev/null || usbip --version 2>/dev/null || echo 'not found'")
            version_output = stdout.read().decode().strip()
            
            if "not found" in version_output:
                return False, "USB/IP tools not installed. Install with: sudo apt install linux-tools-generic (Ubuntu) or equivalent", ""
            
            # Check for systemd service
            stdin, stdout, stderr = ssh_client.exec_command("systemctl list-unit-files | grep usbipd || echo 'no service'")
            service_output = stdout.read().decode().strip()
            
            if "no service" in service_output:
                return False, "usbipd systemd service not found. USB/IP may not be properly installed", version_output
            
            # Check kernel modules availability
            stdin, stdout, stderr = ssh_client.exec_command("modinfo usbip_host usbip_core 2>/dev/null | grep -E '^filename:' | wc -l")
            module_count = stdout.read().decode().strip()
            
            if int(module_count) < 2:
                return False, "USB/IP kernel modules not available. Install linux-tools or usbip packages", version_output
            
            return True, f"USB/IP installation verified - {version_output}", version_output
    ...
    def load_kernel_modules(ssh_client, password):
        ...
        try:
            ...
            if stderr_output and "failed" in stderr_output.lower() and "already loaded" not in stderr_output.lower():
                return False, f"Failed to load kernel modules: {stderr_output}"
            
            # Verify modules are loaded
            stdin, stdout, stderr = ssh_client.exec_command("lsmod | grep -E 'usbip_host|usbip_core' | wc -l")
    ...
    def unload_kernel_modules(ssh_client, password):
        ...
        try:
            ...
            unload_cmd = SecureCommandBuilder.build_modprobe_command("-r usbip_host usbip_core", password, remote_execution=True)
            if not unload_cmd:
            ...
            if int(loaded_count) == 0:
                ...
                return False, "⚠️ Some modules may still be in use and cannot be unloaded"
